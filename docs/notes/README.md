...
| [`flight-multi-update.md`](./flight-multi-update.md) | Empirical findings on multi-write Flight slots (duplicate rows are fatal; stream-typed slots are the sanctioned mechanism) and the per-parton multiplexed-payload design that follows — prototype in `framework/src/lib/parton-mux.ts`. |
