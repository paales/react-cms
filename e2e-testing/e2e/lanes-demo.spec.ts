import { expect, test, waitForLiveConnection, waitForPageInteractive } from "./fixtures"
...
test("clock lanes keep ticking while a slow parton's lane renders", async ({ page }) => {
  ...
  await page.getByTestId("lanes-demo-bump").and(page.locator("[data-hydrated]")).click()
  ...
  const inside = [...seenSeconds].map(Number).filter((s) => s > startedSecond && s < finishedSecond)
  expect(
    inside.length,
...
test("the slow lane's content itself arrives correctly after the overlap", async ({ page }) => {
  await page.goto("/lanes-demo")
  await waitForPageInteractive(page)
  await waitForLiveConnection(page)
  ...
  const slow = page.getByTestId("lanes-demo-slow")
  await expect(slow).toHaveAttribute("data-slow-version", "0")
  const bump = page.getByTestId("lanes-demo-bump").and(page.locator("[data-hydrated]"))
