 ...
 *   4. session lifecycle — a report for a connection that was never
 ...
 *   5. report ordering — a stale report (older seq) can't regress the
...
describe("connection-session visibility", () => {
	...
	it("a flip for an id the route never rendered is dropped without disturbing the stream", async () => {
		...
		await withLiveDrive(
			...
			async (h) => {
				...
				// A report naming an unknown id must not open a lane; the next
				// real flip still lanes normally on the same connection.
				expect(
