	...
	it("moves the session frame URL and lanes the frame's targets on the open region", async () => {
		...
		await withLiveDrive(
			...
			async (h) => {
				...
				expect((await decodeLane(lane)).bodyText).toContain(
					"panel:/panel/b:2",
				);
				...
				expect(
					_peekConnectionSession(conn)?.consumedFrameNavSeqs.get(
						"frame-panel",
					),
				).toBe(2);
	...
	it("a cancel-then-url envelope aborts the stalled frame render — one settled covering lane", async () => {
		...
		await withLiveDrive(
			...
			async (h) => {
				...
				expect((await decodeLane(covering)).bodyText).toContain(
					"panel:/panel/c:",
				);
				const navFlagged = h.entries.filter(
					(e) =>
						(e.tag === "seq" || e.tag === "muxlive") &&
						e.body.startsWith("frame-panel\n"),
				);
				...
				expect(
					_peekConnectionSession(conn)?.cancelSeqByScope.get("frame-panel"),
				).toBe(4);
	...
	it("a window navigation voids a reservation whose lane it tears — the seqvoid entry ships", async () => {
		...
		await withLiveDrive(
			...
			async (h) => {
				...
				const voided = await waitForEntry(
					h.entries,
					(e) => e.tag === "seqvoid",
					"seqvoid entry",
				);
