/**
 * Per-parton multiplexed Flight frames — wire-format prototype.
 *
 ...
 * parton's update waits on a slower sibling's Suspense boundary. This
 * is the transport half of the design in
 * `docs/notes/flight-multi-update.md`; it is exercised by
 * `__tests__/parton-mux.rsc.test.tsx` and not yet wired into any
 * response path.
 *
 ...
 *
 * The tags are local to this prototype; when a response path adopts
 * the format they move to the tag taxonomy in `fp-trailer-marker.ts`.
 */
...
function muxFrame(partonId: string, chunk: Uint8Array): Uint8Array {
  const idBytes = ENCODER.encode(partonId)
  const marker = buildMarker(TAG_MUX_FRAME, idBytes.byteLength + 1 + chunk.byteLength)
  const out = new Uint8Array(marker.byteLength + idBytes.byteLength + 1 + chunk.byteLength)
  out.set(marker, 0)
...
function muxEndFrame(partonId: string): Uint8Array {
  const idBytes = ENCODER.encode(partonId)
 ...
 */
export function muxPartonStreams(sources: PartonSource[]): ReadableStream<Uint8Array> {
  for (const s of sources) {
    if (s.partonId.includes("\n")) {
      throw new Error(`parton id must not contain newline: ${JSON.stringify(s.partonId)}`)
    }
  }
        ...
        sources.map(async ({ partonId, stream }) => {
          ...
          try {
            while (true) {
              ...
              if (done) break
              if (value && value.byteLength > 0) controller.enqueue(muxFrame(partonId, value))
            }
          } finally {
            reader.releaseLock()
          }
    ...
    async *[Symbol.asyncIterator]() {
      ...
      try {
        frames: while (true) {
          ...
          if (parsed === "invalid") throw new Error("mux stream: invalid frame marker")
          const total = parsed.headerSize + parsed.length
          while (buffer.byteLength < total) {
            if (!(await readMore())) throw new Error("mux stream ended mid-frame body")
          }
          ...
          if (parsed.tag !== TAG_MUX_FRAME) {
            throw new Error(`mux stream: unexpected frame tag ${JSON.stringify(parsed.tag)}`)
          }
        ...
        const torn = new Error("mux stream closed with parton payload incomplete")
