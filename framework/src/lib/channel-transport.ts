...
export class WebSocketTransport implements ChannelTransport {
  ...
  open(
    ...
    try {
      ...
      return Promise.reject(new NavigationError({ kind: "network", url, cause: err }))
    }
    ...
    return new Promise<{ body: ReadableStream<Uint8Array> }>((resolve, reject) => {
      let opened = false
...
export function selectChannelTransport(): void {
  ...
  if (requested === "webtransport") {
    ...
    if (typeof WebTransport !== "undefined") setChannelTransport(new WebTransportTransport())
    return
  }
  if (requested === "ws") {
    transportForced = true
    if (typeof WebSocket !== "undefined") setChannelTransport(new WebSocketTransport())
    return
  }
  if (requested === "fetch") {
    transportForced = true
...
export function probeWebSocketTransport(
  ...
  return new Promise<boolean>((resolve) => {
    ...
    const timer = setTimeout(() => finish(false), opts.timeoutMs ?? PROBE_CONN_TIMEOUT_MS)
    probe.open(statement, abort.signal).then(
      ({ body }) => {
        void (async () => {
          try {
