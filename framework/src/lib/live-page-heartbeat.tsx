 ...
 *  segmented stream (see `../entry/live-boot.tsx`). */
...
/**
 * Install the live-connection heartbeat — each fire is an ATTACH: a
 * `POST /__parton/live` whose body carries the full client statement
 * (URL + manifest + catch-up anchor + viewport seed + any
 * pre-establishment intent; see `channel-protocol.ts`), answered by the
 * held segmented stream. Installed IMPERATIVELY by the live layer
 * (`../entry/live-boot.tsx`), which the browser bootstrap dynamically
 * imports ONCE post-commit — so both ordering preconditions the first
 * fire needs are already satisfied by install time:
 *   1. React's first commit has flushed — `PartialErrorBoundary`s have
 *      populated `_currentPageFingerprints` with the cold fps (the
 *      live layer loads from a post-commit `useEffect`).
 *   2. The browser `load` event — which parses the SSR HTML trailer
 *      comment after `</html>` and applies its warm-fp drift
 *      corrections (`_applyFpTrailerFromDocument` registers a `load`
 *      listener). Without this the attach's manifest carries only cold
 *      fps; a parton whose cold fp drifted from warm re-renders — a
 *      visible flash for time-dependent content (`new Date()`). This
 *      installer still gates its FIRST fire on `readyState` /
 *      `load` (below), so the drift corrections always precede it.
 * A pre-establishment INTERACTION jumps both queues: the channel
 * transport requests an immediate fire (`_registerAttachRequester`) so
 * the statement rides the attach it triggered — a cold-manifest attach
 * over-fetches, never waits.
 *
 * Behaviour:
 *   - Install → one attach fire; the transport (channel-client)
 ...
 *
 * Returns a teardown fn (the live layer holds it for the page
 * lifetime; a dev HMR dispose calls it).
 */
export function installLivePageHeartbeat(intervalMs: number = DEFAULT_INTERVAL_MS): () => void {
  ...
  const fire = () => {
    ...
    // This installer contributes the anchor + seed + watermark
