...
export function createProxyNode(
	...
	return new Proxy(target, {
		get(_target, prop, _receiver) {
			...
			if (childHasData && childData != null && typeof childData === "object" && !Array.isArray(childData)) {
			...
			if (childHasData) {
				return createProxyNode(schema, childTypeName, childPath, recorder, childData);
			}
		...
		apply(_target, _thisArg, argsList) {
			...
			return createProxyNode(schema, typeName, path, recorder, internals.hasData ? internals.data : undefined);
		},
	});
}
